...
# Google の口コミ（2026-09-24 受領 → 2026-09-24 プライバシー配慮で見直し）
# 方針：
#  - 投稿者名は出さない（姓のみでも、口コミ本文と結びつくと個人が特定されうるため）
#  - ご家族の構成や個人的な思い出など、サービスと関係のない記述は載せない
#  - サービスについて書かれた部分だけを抜粋する。抜粋であることはページに明記する
#  - 全文は Google の口コミ一覧で見られるようにリンクを置く
# ※ 構造化データ（Review / AggregateRating）は入れない。
#    自社サイトに自社の評価を出すマークアップは Google のガイドライン違反になるため。
VOICES = [
    "些細な事や、無理な注文にも対応して頂き感謝してます。全メーカーの車種に対応しているので安心です。",
    "決して敷居が高くなく、いつもユーザー目線なのが好きです。新車はもちろん、修理も確かで、難しい相談でも乗ってくれるのが有り難いですね。",
    "営業マン、サービスの方が大変親切で丁寧に対応してくれます。",
]
...
SERVICE_FAQ = {
    # 答え：新車は国産全メーカー／中古車は展示車＋業者オークションのオーダー形式／買取・下取り
    "cars": ["欲しい車種が決まっているのですが、注文できますか？",
             "中古車も探してもらえますか？",
             "いまのおクルマは売れますか？"],
    # 答え：メンテナンス／ファイナンスの比較表。ページ名のとおり個人も可
    "lease": ["車検や税金も月々の支払いに含められますか？",
              "リースと購入は何が違いますか？",
              "個人でも借りられますか？"],
    ...
    "bodywork": ["小さな傷やへこみでも直してもらえますか？",
                 "保険を使って直せますか？",
                 "コーティングもお願いできますか？"],
    # 答え：2社から比べて選べます／事故が起きたときの流れ／修理も自社工場で対応
    "insurance": ["いま入っている保険を見直せますか？",
                  "事故を起こしたら、まず何をすればいいですか？",
                  "修理までお願いできますか？"],
}

# 並び順は「来る人が多い順」。車検・整備を探して来る方が最も多いので先頭に置く
# （2026-09-26 変更。それまでは車販が先頭で、需要と逆だった）
# トップのサービス欄に添える「決め手になる事実」。
# サービスページの料金表で裏が取れているものだけを書く（2026-09-26）。
# 数字を変えるときは、必ず先にページ側の表を直すこと。
SERVICE_FACT = {
    # 裏づけ：ニュースマイル車検 料金表（軽・ニューサービスコース）と FAQ
    "inspection": "軽自動車 50,760円〜（総額・税込）。60日前のご予約で 2,200円引き。"
                  "スマイル・プレミアムコースは、ご来店時のレンタカーが無料です。",
    # 裏づけ：PGⅢ「親水性」コーティング 施工価格表（Aクラス）
    "bodywork": "PGⅢコーティングは新車時 55,000円〜（税込）。傷・へこみのお見積りは無料です。",
}

SERVICES = [
    # slug, 番号, 名称, 1行説明, 画像, 写真タグ
    ("inspection", "01", "車検・点検・整備", "他社で買ったおクルマでも、そのまま見ます。国産車は全メーカー、輸入車もお受けします。", "inspection", ""),
...
def cta(root):
  ...
  <div class="wrap">
    <div>
      <h2>ご相談は無料なので、お気軽に<br>お問い合わせください</h2>
...
# ======================================================================
def voices_html():
    """Google の口コミ。投稿者名は出さず、サービスについて書かれた部分だけを抜粋する"""
    items = "".join(f'\n      <li class="voice"><blockquote>{t}</blockquote></li>' for t in VOICES)
    return f'''
<section class="sec" id="voice">
  <div class="wrap">
    <span class="eyebrow">VOICE</span>
    <h2 class="sec-title">お客様の声</h2>
    <p class="lead">Google にいただいた口コミから、サービスについてのご意見を抜粋しました。お名前は掲載していません。</p>
    <ul class="voices">{items}
    </ul>
    <p class="muted" style="margin-top:18px">一部を抜粋しています。全文と投稿者は Google でご覧いただけます。</p>
...
def build_index():
    ...
    <article class="svc-row">
      ...
      <div>
        ...
        {f'<p class="svc-row__fact">{SERVICE_FACT[slug]}</p>' if slug in SERVICE_FACT else ""}
        <ul class="tr-list">
  ...
  <div class="wrap">
    <div>
      ...
      <ul class="pills">
        ...
        <li><span>他社で買ったおクルマも<em>そのまま見ます</em></span></li>
      </ul>
      <div class="btn-row">
...
<section class="sec" id="about">
  <div class="wrap split">
    <div>
      ...
      <p class="lead">タカヤモーター（昭和40年創立）とタカヤリース（昭和59年創立）の2社で、販売から車検・整備、板金、保険までを同じ場所で扱っています。前回どこを直したか、どんな乗り方をされているかが分かるので、そのおクルマに合った提案ができます。</p>
...
<section class="sec sec--alt" id="service" data-area="service">
  <div class="wrap">
    ...
    <h2 class="sec-title">どこに頼めばいいか分からないときは、ここから</h2>
    <p class="lead">「これって、何をお願いすればいいの」という状態でも大丈夫です。実際によくいただくご質問を並べました。</p>
...
def build_services_index():
    ...
    body = page_head(root, [("index.html", "トップ"), (None, "サービス")], "サービス",
                     "おクルマを探す・買い取る、リース、車検・点検・整備、板金・コーティング、自動車保険。おクルマに関することは、この5つのサービスでまとめてお受けします。") + f'''
<section class="sec"><div class="wrap">
  <h2 class="sec-title">5つのどれでも、窓口は同じです</h2>
...
<div class="box-grid">
  <div class="box">
    ...
    <ul class="dash-list">
      ...
      <li>車両本体からオプション・諸費用まで、<strong>お見積りは1枚にまとめて</strong>お出しします。</li>
    </ul>
  </div>
  <div class="box">
...
<p class="lead"><strong>東京海上日動・損保ジャパンの代理店</strong>として、自動車保険をお取り扱いしています。おクルマを買ったお店・車検を受けているお店で保険も相談できるので、お車の使い方に合った補償を選べます。</p>
...
def build_company():
    ...
    body = page_head(root, [("index.html", "トップ"), (None, "会社情報")], "会社情報",
                     "タカヤモーター株式会社（昭和40年創立）とタカヤリース株式会社（昭和59年創立）。岡山市中区高屋で、1965年から延べ10万台以上のおクルマを見てきました。") + f'''
